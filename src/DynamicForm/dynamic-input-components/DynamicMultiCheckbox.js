...
type MultiCheckboxFieldTypeConfiguration = {
  options: string[]
}
...
> {
  ...
  render() {
    const {name, value, onChange, onBlur, fieldTypeConfiguration} = this.props
    console.log(value)
    return (
      <div>
        {fieldTypeConfiguration.options.map((option, index) => (
          <label key={index}>
            ...
            <input
              name={name}
              onChange={this.handleChange(option, value.some(x => x === option))}
              type="checkbox"
            />
          </label>
        ))}
